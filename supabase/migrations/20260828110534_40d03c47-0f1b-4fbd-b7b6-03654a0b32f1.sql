...
RETURNS uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT tenant_id FROM public.portal_users
  WHERE user_id = _user_id AND _user_id = auth.uid() AND status = 'active' LIMIT 1
$$;
...
RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT portal_role FROM public.portal_users
  WHERE user_id = _user_id AND _user_id = auth.uid() AND status = 'active' LIMIT 1
$$;
    ...
    WHERE run.id = _run_id
      ...
      AND (
        ...
        OR (
          ...
          AND EXISTS (
            SELECT 1 FROM public.payslips payslip
    ...
    AND (
      ...
      OR (
        ...
        AND EXISTS (
          SELECT 1 FROM public.payslips payslip
          JOIN public.payroll_employees employee ON employee.id = payslip.employee_id
          WHERE payslip.document_id = documents.id AND employee.user_id = auth.uid()
        )
      )
    )
  );
  ...
  USING (
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE employee.employer_id = payroll_employers.id AND employee.user_id = auth.uid()
      )
    )
  );
          ...
          OR (
            ...
            AND (
              ...
              OR EXISTS (
                SELECT 1 FROM public.payslips payslip
                JOIN public.payroll_employees employee ON employee.id = payslip.employee_id
                WHERE payslip.document_id = document.id AND employee.user_id = auth.uid()
              )
            )
          )
        )
...
-- Repair staging identities. The tenant-immutability trigger is suspended for
-- this maintenance block only.
ALTER TABLE public.profiles DISABLE TRIGGER trg_profiles_prevent_tenant_change;
...
BEGIN
  ...
  WHERE lower(account.email) = 'client@kitchen313.dev' AND portal.status = 'active'
  LIMIT 1;
...
ALTER TABLE public.profiles ENABLE TRIGGER trg_profiles_prevent_tenant_change;
