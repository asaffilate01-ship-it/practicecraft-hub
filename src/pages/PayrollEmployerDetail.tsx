    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">PAYE Ref: {employer?.paye_reference} · Accounts Office: {employer?.accounts_office_ref}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card><CardContent className="pt-4"><p className="text-xs text-muted-foreground">Employer Name</p><p className="text-2xl font-bold">{employer?.employer_name || "—"}</p></CardContent></Card>
      </div>
            ...
            <div className="space-y-2">
              {employees.map((emp: any) => (
                <div key={emp.id} className="flex items-center justify-between py-2 border-b last:border-0">
                  ...
                  <Badge variant="secondary">{emp.employment_status || "active"}</Badge>
                </div>
              ))}
            </div>
          )}
