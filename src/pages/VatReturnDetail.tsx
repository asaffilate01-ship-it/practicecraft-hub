  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <Badge variant={vatReturn?.status === "submitted" ? "default" : "secondary"} className="ml-auto">{vatReturn?.status || "draft"}</Badge>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Box 1 – VAT due on sales", value: vatReturn?.box1 },
          { label: "Box 2 – VAT due on acquisitions", value: vatReturn?.box2 },
          { label: "Box 3 – Total VAT due", value: vatReturn?.box3 },
          { label: "Box 4 – VAT reclaimed", value: vatReturn?.box4 },
          { label: "Box 5 – Net VAT", value: vatReturn?.box5 },
          { label: "Box 6 – Total sales (excl VAT)", value: vatReturn?.box6 },
          { label: "Box 7 – Total purchases (excl VAT)", value: vatReturn?.box7 },
      ...
      <Card>
        <CardHeader><CardTitle className="text-base flex items-center gap-2"><FileText className="w-4 h-4" /> Submission Details</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">HMRC Receipt</span><span>{vatReturn?.hmrc_receipt || "—"}</span></div>
